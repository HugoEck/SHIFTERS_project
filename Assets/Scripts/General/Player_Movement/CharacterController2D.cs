...
public abstract class CharacterController2D : MonoBehaviour
{
    ...
    [SerializeField] protected bool _bCanAirControl = false;                         // Whether or not a player can steer while jumping;
    ...
    protected bool _bIsGrounded;            // Whether or not the player is grounded.
    protected bool _bIsfacingRight = true;  // For determining which way the player is currently facing.    
    ...
    public void Move(float move, bool crouch, bool jump)
    {
        ...
        if (_bIsGrounded || _bCanAirControl)
        {
            ...
            if (move > 0 && !_bIsfacingRight)
            {
                // ... flip the player.
                Flip();
            }
            // Otherwise if the input is moving the player left and the player is facing right...
            else if (move < 0 && _bIsfacingRight)
            {
                // ... flip the player.
                Flip();
            }
